...
8.5) J'ai conçu le router (users_router.py) pour isoler totalement l'exposition HTTP de la logique métier. J'ai utilisé l'objet APIRouter pour déclarer les points d'entrée /users (GET et POST) et /users/{user_id} (GET). Conformément au principe de séparation des responsabilités, mon router n'instancie aucune classe, il reçoit une instance de UsersService par injection via la fonction Depends. Pour la route de création, j'ai configuré le router pour qu'il réceptionne un objet UserModelCreate qu'il transmet directement au service. Enfin, j'ai intégré la validation HTTP en levant une HTTPException avec un code 404 dans les cas où le service renvoie une valeur nulle.
J'ai placé la logique d'instaciation du service dans une fonction de dépendance qui instancie les Settings et la factory, puis les injecte dans le constructeur du UsersService. J'ai appliqué le décorateur @lru_cache sur cette fonction pour transformer mon service en un composant unique (Singleton) qui conserve sa liste d'utilisateurs en mémoire, évitant ainsi des lectures fichiers répétitives.

8.7) Dans le fichier main.py, j'ai créé une fonction create_app qui instancie FastAPI et rattache mon router via app.include_router(). Cette structure me permet de piloter l'ensemble de l'application depuis un point unique. Pour le lancement, j'utilise la commande uvicorn app.main:app depuis la racine, ce qui garantit la résolution correcte de tous mes imports absolus au sein du projet.